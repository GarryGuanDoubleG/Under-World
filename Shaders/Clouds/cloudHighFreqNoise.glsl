...
void main() {
 // The single most important value to get right. All other values should be hardcoded,
	...
	// Texture 2
	float texture2_low              = -0.5;
	float texture2_high             =  1.0;

	ivec3 coord = ivec3(gl_GlobalInvocationID.xyz);
	vec3 fragCoord = vec3(coord) / vec3(imageSize(DestTex).xyz);
	...
	float worley_value_r = get_worley_3_octaves(fragCoord, 5);
	float worley_value_g = get_worley_3_octaves(fragCoord, 8);
	float worley_value_b = get_worley_3_octaves(fragCoord, 10);
	float worley_value_a = get_worley_3_octaves(fragCoord, 5);
	...
	//vec4 result = vec4(1.0f) - vec4(worley_value_r, worley_value_g, worley_value_b, worley_value_a);
